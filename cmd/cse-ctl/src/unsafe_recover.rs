...
pub(crate) fn execute_unsafe_recover(matches: &ArgMatches) {
    let path = matches.value_of_os("path").unwrap();
    let rf = rfengine::RfEngine::open(Path::new(path), 512 * 1024 * 1024).unwrap();
    let target_regions = if let Some(region_id_str) = matches.value_of("region") {
        let region_id = u64::from_str(region_id_str).unwrap();
    ...
    } else if let Some(keyspace_id_str) = matches.value_of("keyspace") {
        let keyspace_id = u32::from_str(keyspace_id_str).unwrap();
        let mut keyspace = keyspace_id.to_be_bytes();
        keyspace[0] = 'x' as u8;
        let mut prefix = keyspace.to_vec();
        if let Some(table_id_str) = matches.value_of("table") {
            let table_id = u64::from_str(table_id_str).unwrap();
            prefix.put_u8('t' as u8);
            prefix.encode_i64(table_id as i64).unwrap();
        }
        collect_prefix_regions(&rf, &prefix)
    } else if let Some(table_id_str) = matches.value_of("table") {
        let table_id = u64::from_str(table_id_str).unwrap();
    ...
    if let Some(stores_str) = matches.value_of("remove-stores") {
        let failed_stores = parse_stores(stores_str);
    ...
    } else if matches.is_present("destroy") {
        for (peer_id, region_id, region_version) in target_regions {
