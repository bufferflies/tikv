...
use kvproto::{kvrpcpb, kvrpcpb::KeyRange, raft_serverpb::*};
...
fn test_extra_snapshot_override() {
    ...
    // snap_ctx.secondary_region_override.as_mut().unwrap().peer =
    // r2_old_leader.clone(); assert!(!snap_ctx.pb_ctx.replica_read);
    // let err = cluster
    //     .must_get_raft_engine(r0_leader.get_store_id())
    //     .snapshot(snap_ctx.clone())
    //     .err()
    //     .unwrap();
    // assert_matches!(err, Error(box ErrorInner::Request(header)) if
    // header.not_leader.is_some());
}
