...
fn test_replication_worker() {
    ...
    must_wait(
        || {
            ...
            info!("Current region count: {}", region_count);
            region_count == 9
        },
        30,
        || "wait for region merge".into(),
    );
    let mut tidb_opts = StartTidbOptions::default();
    tidb_opts.enable_system_tidb = true;
    block_on(tc.start_tidb(1, Duration::from_secs(300), "info", tidb_opts));
    ...
    must_wait(
        || {
            let region_count = block_on(pd_ctl.get_regions_number()).unwrap();
            info!("wait for region split, region count {}", region_count);
            region_count == 10
        },
        10,
