...
use rfstore::Result;
use test_raftstore::{
    must_contains_error, new_admin_request, new_change_peer_request, new_change_peer_v2_request,
    new_learner_peer, new_peer, sleep_ms,
};
use test_rfstore::*;
...
fn test_joint_consensus_conf_change() {
    ...
    must_get_equal(&cluster.get_engine(2), region_id, b"k1", b"v1");
    must_get_equal(&cluster.get_engine(3), region_id, b"k1", b"v1");
    must_get_equal(&cluster.get_engine(4), region_id, b"k1", b"v1");
    ...
    shard_must_not_exist(&cluster.get_engine(4), region_id);
    ...
    pd_client.must_leave_joint(region_id);
    shard_must_not_exist(&cluster.get_engine(3), region_id);
    must_get_equal(&cluster.get_engine(4), region_id, b"k1", b"v1");
}
...
fn test_request_in_joint_state() {
    ...
    must_get_equal(&cluster.get_engine(2), region_id, b"k2", b"v2");
    must_get_equal(&cluster.get_engine(3), region_id, b"k2", b"v2");
    ...
    must_get_equal(&cluster.get_engine(3), region_id, b"k5", b"v5");
}
...
fn test_joint_replace_peers() {
    ...
    cluster.must_put(b"k2", b"v2");
    must_get_equal(&cluster.get_engine(2), region_id, b"k2", b"v2");
    must_get_none(&cluster.get_engine(3), region_id, b"k2");
    must_get_none(&cluster.get_engine(4), region_id, b"k2");
    ...
    cluster.clear_send_filters();
    must_get_equal(&cluster.get_engine(3), region_id, b"k2", b"v2");
    must_get_equal(&cluster.get_engine(4), region_id, b"k2", b"v2");
    ...
    must_get_equal(&cluster.get_engine(5), region_id, b"k3", b"v3");

    cluster.must_transfer_leader(region_id, new_peer(5, 5));

    cluster.must_put(b"k4", b"v4");
    for id in 1..=5 {
        must_get_equal(&cluster.get_engine(id), region_id, b"k4", b"v4");
    }
...
fn test_leader_down_in_joint_state() {
    ...
    for i in 2..=5 {
        must_get_equal(&cluster.get_engine(i), region_id, b"k1", b"v1");
    }
    ...
    cluster.must_put(b"k2", b"v2");
    for i in 2..=5 {
        must_get_equal(&cluster.get_engine(i), region_id, b"k2", b"v2");
    }
...
fn put_request(region: &Region, id: u64, key: &[u8], val: &[u8]) -> RaftCmdRequest {
    let mut request = new_write_request(
        region.get_id(),
