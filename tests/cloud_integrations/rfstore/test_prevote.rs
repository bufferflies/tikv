...
use test_raftstore::{
    configure_for_lease_read, new_peer, new_region_leader_cmd, new_status_request,
};
use test_rfstore::*;
use tikv_util::{config::ReadableDuration, debug, HandyRwLock};
...
// themselves.
#[test]
fn test_pair_isolated() {
    let mut cluster = new_node_cluster(0, 5);
    cluster.cfg.raft_store.max_leader_missing_duration = ReadableDuration::millis(3000);
    cluster.cfg.raft_store.abnormal_leader_missing_duration = ReadableDuration::millis(1500);
...
#[test]
fn test_isolated_follower_leader_does_not_change() {
    let mut cluster = new_node_cluster(0, 5);
    cluster.run();
...
#[test]
#[ignore = "See: https://github.com/tidbcloud/cloud-storage-engine/issues/2441"]
// skipped due to current implementation of rfstore.
fn test_create_peer_from_pre_vote() {
    let mut cluster: Cluster<NodeCluster> = new_node_cluster(0, 2);
    cluster.cfg.raft_store.prevote = true;
    let pd_client = Arc::clone(&cluster.pd_client);
    pd_client.disable_default_operator();
