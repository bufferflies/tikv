...
use test_raftstore::{
    configure_for_lease_read, configure_for_snapshot, new_admin_request, new_change_peer_request,
    new_learner_peer, new_peer, new_region_leader_cmd, new_status_request, sleep_ms,
};
use test_rfstore::*;
use tikv_util::{
    box_err,
...
macro_rules! call_conf_change {
    ($cluster:expr, $region_id:expr, $conf_change_type:expr, $peer:expr) => {{
...
fn test_server_simple_conf_change() {
    ...
    shard_must_not_exist(&engine_2, r1);
    ...
    must_get_equal(&engine_2, r1, b"k1", b"v1");
    must_get_equal(&engine_2, r1, b"k2", b"v2");
    ...
    shard_must_not_exist(&engine_5, r1);
    ...
    must_get_equal(&engine_3, r1, b"k1", b"v1");
    must_get_equal(&engine_3, r1, b"k2", b"v2");
    must_get_equal(&engine_3, r1, b"k3", b"v3");
    ...
    must_get_equal(&engine_2, r1, b"add_2_4", b"add_2_4");
    ...
    shard_must_not_exist(&engine_3, r1);
...
fn test_pd_conf_change() {
    ...
    shard_must_not_exist(&engine_2, region_id);
    ...
    // now peer 2 must have v1 and v2;
    must_get_equal(&engine_2, region_id, b"k1", b"v1");
    must_get_equal(&engine_2, region_id, b"k2", b"v2");
    ...
    must_get_equal(&engine_3, region_id, b"k1", b"v1");
    ...
    // now peer 3 must have v1, v2 and v3
    must_get_equal(&engine_3, region_id, b"k1", b"v1");
    must_get_equal(&engine_3, region_id, b"k2", b"v2");
    must_get_equal(&engine_3, region_id, b"k3", b"v3");
    ...
    must_get_equal(&engine_2, region_id, b"k1", b"v1");
    must_get_equal(&engine_2, region_id, b"k4", b"v4");

    // peer 3 has nothing, we check v1 and v4 here.
    shard_must_not_exist(&engine_3, region_id);
...
fn test_auto_adjust_replica() {
    ...
    wait_till_reach_count(Arc::clone(&pd_client), region_id, 3);
    ...
    for peer in region.get_peers() {
        must_get_equal(
            &cluster.get_engine(peer.get_store_id()),
            region_id,
            b"k1",
            b"v1",
        );
    ...
    wait_till_reach_count(Arc::clone(&pd_client), region_id, 2);
...
macro_rules! find_leader_response_header {
    ($cluster:expr, $region_id:expr, $peer:expr) => {{
...
fn test_after_remove_itself() {
    ...
    pd_client.must_add_peer(r1, new_peer(2, 2));
    must_get_equal(&cluster.get_engine(2), r1, b"kk", b"vv");
    pd_client.must_add_peer(r1, new_peer(3, 3));
    must_get_equal(&cluster.get_engine(3), r1, b"kk", b"vv");
    ...
    must_get_equal(&engine1, r1, b"k1", b"v1");
    must_get_equal(&engine3, r1, b"k1", b"v1");
    ...
    let mut req = new_write_request(1, epoch, put);
    ...
    for _ in 0..250 {
        let region = cluster.region_local_state(1, 1).unwrap();
...
fn test_split_brain() {
    ...
    must_get_equal(&cluster.get_engine(1), r1, b"kk0", b"vv0");
    must_get_equal(&cluster.get_engine(2), r1, b"kk0", b"vv0");
    must_get_equal(&cluster.get_engine(3), r1, b"kk0", b"vv0");
    ...
    must_get_equal(&cluster.get_engine(4), r1, b"k1", b"v1");
    pd_client.must_add_peer(r1, new_peer(5, 5));
    must_get_equal(&cluster.get_engine(5), r1, b"k1", b"v1");
    pd_client.must_add_peer(r1, new_peer(6, 6));
    must_get_equal(&cluster.get_engine(6), r1, b"k1", b"v1");
    cluster.must_transfer_leader(r1, new_peer(6, 6));
    pd_client.must_remove_peer(r1, new_peer(2, 2));
    pd_client.must_remove_peer(r1, new_peer(3, 3));

    cluster.must_put(b"k2", b"v2");
    must_get_equal(&cluster.get_engine(6), r1, b"k2", b"v2");
...
fn test_conf_change_safe() {
    ...
    must_get_equal(&cluster.get_engine(2), region_id, b"k0", b"v0");
    pd_client.must_add_peer(region_id, new_peer(3, 3));
    must_get_equal(&cluster.get_engine(3), region_id, b"k0", b"v0");
...
fn test_node_learner_conf_change() {
    ...
    cluster.must_put(b"k2", b"v2");
    must_get_equal(&engine_4, r1, b"k1", b"v1");
    must_get_equal(&engine_4, r1, b"k2", b"v2");
    ...
    shard_must_not_exist(&engine_4, r1); // Wait for the region is cleaned.
    ...
    cluster.must_put(b"k3", b"v3");
    must_get_equal(&cluster.get_engine(4), r1, b"k3", b"v3");
    ...
    cluster.must_put(b"k3", b"v3");
    must_get_equal(&engine_4, r1, b"k3", b"v3");
...
fn test_learner_with_slow_snapshot() {
    ...
    // Set a small gc size limit to trigger log gc.
    cluster.cfg.raft_store.raft_log_gc_size_limit = Some(ReadableSize(100));
    let pd_client = Arc::clone(&cluster.pd_client);
    pd_client.disable_default_operator();
    let r1 = cluster.run_conf_change();
    ...
    cluster.add_send_filter_on_node(1, snap_filter);
    ...
    // We must write more than `RAFT_LOG_BLOCK_CAP`(255) kv to ensure raft log gc is
    // tirggered.
    (0..300).for_each(|_| cluster.must_put(b"k2", b"v2"));
...
fn test_node_stale_peer() {
    ...
    let r1 = cluster.run_conf_change();
    cluster.must_put(b"k1", b"v1");
    must_get_equal(&cluster.get_engine(1), r1, b"k1", b"v1");
    ...
    cluster.clear_send_filters();
    shard_must_not_exist(&cluster.get_engine(3), r1);
}
...
#[test]
#[ignore = "waiting for cherry pick PR https://github.com/tikv/tikv/issues/16174"]
