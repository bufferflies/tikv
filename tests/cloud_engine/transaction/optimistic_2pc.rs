...
use test_cloud_server::{
    client::{ClusterClient, Error, TxnMutations},
    ServerCluster,
};
...
fn setup_initial_value(
    ...
    let commit_ts_init = client
        .prewrite_and_commit(start_ts_init, &init_mutations_vec, TimeStamp::zero())
...
fn test_opt_write_conflict_key_locked_by_uncommitted_opt_txn() {
    ...
    // Act: T1 prewrites
    let res = client.kv_prewrite_with_retry_opt(
        muts1.primary(),
        None,
        muts1.clone(),
        start_ts1,
...
fn test_opt_write_conflict_during_prewrite_check() {
    ...
    let commit_ts_other = client
        .prewrite_and_commit(start_ts_other, &muts_other_vec, TimeStamp::zero())
    ...
    let res = client.kv_prewrite_with_retry(muts1.primary(), None, muts1.clone(), start_ts1);
...
fn test_opt_rollback_explicitly_after_prewrite() {
    ...
    client
        .kv_prewrite_with_retry(muts_t1.primary(), None, muts_t1.clone(), start_ts_t1)
