...
use crate::storage::{
    ...
    txn::{
        commands::txn_file::TxnFileCommand, latch, txn_status_cache::TxnStatusCache, ProcessResult,
        Result,
    },
    types::{
...
pub(crate) async fn find_mvcc_infos_by_key<S: Snapshot>(
    ...
    let mut writes = reader.cloud_reader.scan_write_for_key(key, ts)?;
    writes.extend(reader.cloud_reader.get_extras(key));
