...
use test_raftstore::{
    configure_for_causal_ts, configure_for_lease_read, must_check_txn_status, must_kv_commit,
    new_peer, sleep_ms, try_kv_prewrite_pessimistic,
};
use test_rfstore::*;
use tikv_util::config::ReadableDuration;
use txn_types::{Key, Lock, LockType, TimeStamp};

use crate::{new_event_feed, new_event_feed_v2, CloudTestSuiteBuilder, TestSuite};

#[test]
fn test_cdc_basic() {
    let mut suite = TestSuite::new(1, ApiVersion::V2);

    let region_id = suite.cluster.pd_client().get_region(b"xkey0").unwrap().id;
    ...
    // request again.
    let region_id2 = suite.cluster.pd_client().get_region(b"xkey0").unwrap().id;
    let req = suite.new_changedata_request(region_id);
    let (mut req_tx, resp_rx) = suite
        .get_region_cdc_client(region_id2)
        .event_feed()
        .unwrap();
    ...
    let (mut req_tx, resp_rx) = suite.get_region_cdc_client(region_id).event_feed().unwrap();
    block_on(req_tx.send((req, WriteFlags::default()))).unwrap();
    event_feed_wrap.replace(Some(resp_rx));
    let mut events = receive_event(false).events.to_vec();
    assert_eq!(events.len(), 1);
...
#[test]
#[ignore = "Raw KV API not supported in cloud engine"]
fn test_cdc_rawkv_basic() {
    ...
    suite.must_kv_put(region_id, k, v);
    let mut events = receive_event(false).events.to_vec();
    assert_eq!(events.len(), 1, "{:?}", events);
...
fn test_cdc_not_leader() {
    test_kv_format_impl!(test_cdc_not_leader_impl<ApiV2>);
}

fn test_cdc_not_leader_impl<F: KvFormat>() {
    let mut suite = TestSuite::new(3, F::TAG);

    let region_id = suite.cluster.pd_client().get_region(b"").unwrap().id;
    ...
    sleep_ms(1000);
    // There must be a delegate.
    let scheduler = suite.endpoints.get(&leader.get_store_id()).unwrap().clone();
    ...
    let peer = suite
        ...
        .find(|p| *p != leader)
        .unwrap();
    suite.cluster.must_transfer_leader(region_id, peer.clone());
    let mut events = receive_event(false).events.to_vec();
    assert_eq!(events.len(), 1);
    match events.pop().unwrap().event.unwrap() {
        Event_oneof_event::Error(err) => {
...
fn test_cdc_cluster_id_mismatch() {
    test_kv_format_impl!(test_cdc_cluster_id_mismatch_impl<ApiV2>);
}

fn test_cdc_cluster_id_mismatch_impl<F: KvFormat>() {
    let mut suite = TestSuite::new(3, F::TAG);

    let region_id = suite.cluster.get_region(b"").id;
    // Send request with mismatched cluster id.
    let mut req = suite.new_changedata_request(region_id);
    req.mut_header().set_ticdc_version("5.3.0".into());
    req.mut_header().cluster_id += 1;
...
fn test_cdc_stale_epoch_after_region_ready() {
    test_kv_format_impl!(test_cdc_stale_epoch_after_region_ready_impl<ApiV2>);
}
...
fn test_cdc_stale_epoch_after_region_ready_impl<F: KvFormat>() {
    ...
    req.set_region_epoch(suite.get_context(region_id).take_region_epoch());
    block_on(req_tx.send((req, WriteFlags::default()))).unwrap();
    // Must receive epoch not match error.
    let mut events = receive_event(false).events.to_vec();
    assert_eq!(events.len(), 1);
...
#[test]
fn test_cdc_scan() {
    test_kv_format_impl!(test_cdc_scan_impl<ApiV2>);
}

fn test_cdc_scan_impl<F: KvFormat>() {
    let mut suite = TestSuite::new(3, F::TAG);
    let region_id = suite.cluster.pd_client().get_region(b"xkey1").unwrap().id;
    ...
    suite.must_kv_prewrite(region_id, vec![mutation], k.clone(), start_ts1);
    // Commit
    let commit_ts1 = block_on(suite.cluster.pd_client().get_tso()).unwrap();
    suite.must_kv_commit(region_id, vec![k.clone()], start_ts1, commit_ts1);
    ...
    mutation.key = k.clone();
    mutation.value = v.clone();
    suite.must_kv_prewrite(region_id, vec![mutation], k.clone(), start_ts2);
    ...
    suite.must_kv_commit(region_id, vec![k.clone()], start_ts2, commit_ts2);
    // Prewrite delete
    // Start = 8;
    ...
    mutation.key = k.clone();
    suite.must_kv_prewrite(region_id, vec![mutation], k.clone(), start_ts3);
...
#[test]
#[ignore = "RAW KV API wasn't suupported by Test Cloud Enigne."]
...
fn test_cdc_tso_failure() {
    test_kv_format_impl!(test_cdc_tso_failure_impl<ApiV2>);
}
...
fn test_cdc_tso_failure_impl<F: KvFormat>() {
    ...
    let (mut req_tx, event_feed_wrap, receive_event) =
        new_event_feed(suite.get_region_cdc_client(region_id));
    block_on(req_tx.send((req, WriteFlags::default()))).unwrap();
    // Make sure region is registered.
    let mut events = receive_event(false).events.to_vec();
    assert_eq!(events.len(), 1);
    match events.pop().unwrap().event.unwrap() {
        // Even if there is no write,
    ...
    loop {
        ...
        if let Some(resolved_ts) = event.resolved_ts.as_ref() {
            ...
            assert_eq!(resolved_ts.regions, vec![region_id]);
...
fn test_region_split() {
    let mut suite = CloudTestSuiteBuilder::new()
        .num_nodes(1)
        .after_cluster_bootstrapped(|server| {
            server.cluster.get_pd_client().disable_default_operator()
        })
        .cfg_fun(|_id, cfg| cfg.cdc.hibernate_regions_compatible = false)
        .build();
    ...
    // With cloud enigne, we should use `k1` as the start key was encoded. Or we
    // will get the other region. (Not sure how this case works on TiKV...)
    ...
    loop {
        ...
        if let Some(resolved_ts) = event.resolved_ts.as_ref() {
            ...
            assert!(
                ...
                "rts.regions = {:?}, expected = {:?}",
                resolved_ts.regions,
                vec![region.id, region1.id]
            );
            previous_ts = resolved_ts.ts;
            counter += 1;
        }
...
fn test_duplicate_subscribe() {
    test_kv_format_impl!(test_duplicate_subscribe_impl<ApiV2>);
}
...
fn test_cdc_batch_size_limit() {
    test_kv_format_impl!(test_cdc_batch_size_limit_impl<ApiV2>);
}

fn test_cdc_batch_size_limit_impl<F: KvFormat>() {
    let mut suite = TestSuite::new(1, F::TAG);

    let region_id = suite.cluster.get_region(b"xk").id;
    // Prewrite
    let start_ts = block_on(suite.cluster.pd_client().get_tso()).unwrap();
    ...
    suite.must_kv_prewrite(region_id, vec![m1, m2], k1.clone(), start_ts);
    // Commit
    let commit_ts = block_on(suite.cluster.pd_client().get_tso()).unwrap();
    suite.must_kv_commit(region_id, vec![k1, k2], start_ts, commit_ts);
...
fn test_old_value_basic() {
    test_kv_format_impl!(test_old_value_basic_impl<ApiV2>);
}
...
fn test_old_value_basic_impl<F: KvFormat>() {
    ...
    suite.must_kv_prewrite(region_id, vec![m1], k1.clone(), ts1);
    let ts2 = block_on(suite.cluster.pd_client().get_tso()).unwrap();
    suite.must_kv_commit(region_id, vec![k1.clone()], ts1, ts2);
    // Rollback
    let mut m2 = Mutation::default();
    m2.set_op(Op::Put);
    m2.key = k1.clone();
    ...
    suite.must_kv_prewrite(region_id, vec![m2], k1.clone(), ts3);
    suite.must_kv_rollback(region_id, vec![k1.clone()], ts3);
    // Update value
    let mut m3 = Mutation::default();
    m3.set_op(Op::Put);
    m3.key = k1.clone();
    ...
    suite.must_kv_prewrite(region_id, vec![m3], k1.clone(), ts4);
    let ts5 = block_on(suite.cluster.pd_client().get_tso()).unwrap();
    suite.must_kv_commit(region_id, vec![k1.clone()], ts4, ts5);
    // Lock
    let mut m4 = Mutation::default();
    m4.set_op(Op::Lock);
    m4.key = k1.clone();
    let ts6 = block_on(suite.cluster.pd_client().get_tso()).unwrap();
    suite.must_kv_prewrite(region_id, vec![m4], k1.clone(), ts6);
    let ts7 = block_on(suite.cluster.pd_client().get_tso()).unwrap();
    suite.must_kv_commit(region_id, vec![k1.clone()], ts6, ts7);
    // Delete value and rollback
    let mut m5 = Mutation::default();
    m5.set_op(Op::Del);
    m5.key = k1.clone();
    let ts8 = block_on(suite.cluster.pd_client().get_tso()).unwrap();
    suite.must_kv_prewrite(region_id, vec![m5], k1.clone(), ts8);
    suite.must_kv_rollback(region_id, vec![k1.clone()], ts8);
    // Update value
    ...
    suite.must_kv_prewrite(region_id, vec![m6], k1.clone(), ts10);
    let ts11 = block_on(suite.cluster.pd_client().get_tso()).unwrap();
    suite.must_kv_commit(region_id, vec![k1.clone()], ts10, ts11);
    ...
    suite.must_acquire_pessimistic_lock(region_id, vec![m7.clone()], k1.clone(), ts9, ts12);
    m7.set_op(Op::Del);
    suite.must_kv_pessimistic_prewrite(region_id, vec![m7], k1.clone(), ts9, ts12);
    let ts13 = block_on(suite.cluster.pd_client().get_tso()).unwrap();
    suite.must_kv_commit(region_id, vec![k1.clone()], ts9, ts13);
    // Insert value again
    ...
    suite.must_kv_prewrite(region_id, vec![m8], k1, ts14);
...
// For Copilot: Below are test cases not yet fixed.

#[test]
fn test_old_value_multi_changefeeds() {
    test_kv_format_impl!(test_old_value_multi_changefeeds_impl<ApiV2>);
}
...
fn test_old_value_multi_changefeeds_impl<F: KvFormat>() {
    ...
    suite.must_kv_prewrite(region_id, vec![m2], k1.clone(), ts3);
    let ts4 = block_on(suite.cluster.pd_client().get_tso()).unwrap();
    suite.must_kv_commit(region_id, vec![k1], ts3, ts4);
...
#[test]
#[ignore = "Test Cloud Engine uses actual time to allocate timestamp. Cannot reset to `99`."]
fn test_cdc_resolve_ts_checking_concurrency_manager() {
    test_kv_format_impl!(test_cdc_resolve_ts_checking_concurrency_manager_impl<ApiV2>);
}
...
fn test_cdc_resolve_ts_checking_concurrency_manager_impl<F: KvFormat>() {
    ...
    let cm: ConcurrencyManager = suite.get_txn_concurrency_manager(store_id).unwrap();
...
#[test]
fn test_cdc_1pc() {
    test_kv_format_impl!(test_cdc_1pc_impl<ApiV2>);
}

fn test_cdc_1pc_impl<F: KvFormat>() {
    let mut suite = TestSuite::new(1, F::TAG);
    let region_id = suite.cluster.get_region(b"xkey0").id;
    let req = suite.new_changedata_request(region_id);
    let (mut req_tx, _, receive_event) = new_event_feed(suite.get_region_cdc_client(region_id));
    block_on(req_tx.send((req, WriteFlags::default()))).unwrap();
    ...
    let (k1, v1) = (b"xkeyxk1", b"v1");
    let (k2, v2) = (b"xkeyxk2", &[0u8; 512]);
...
fn test_old_value_1pc() {
    test_kv_format_impl!(test_old_value_1pc_impl<ApiV2>);
}
...
fn test_old_value_1pc_impl<F: KvFormat>() {
    ...
    suite.must_kv_commit(region_id, vec![k1.clone()], 10.into(), 15.into());
...
fn test_old_value_cache_hit() {
    test_kv_format_impl!(test_old_value_cache_hit_impl<ApiV2>);
}
...
fn test_old_value_cache_hit_pessimistic() {
    test_kv_format_impl!(test_old_value_cache_hit_pessimistic_impl<ApiV2>);
}
...
#[test]
#[ignore = "Still need to fix."]
fn test_region_created_replicate() {
    let mut suite = CloudTestSuiteBuilder::new()
        .num_nodes(2)
        .after_cluster_bootstrapped(|c| c.cluster.get_pd_client().disable_default_operator())
        .build();

    let region = suite.cluster.get_region(&[]);
    let leader = suite.cluster.leader_of_region(region.id).unwrap();
    let transfee = region
        .peers
        .iter()
        .find(|p| p.id != leader.id)
        .cloned()
        .unwrap();
    suite.cluster.must_transfer_leader(region.id, transfee);
    suite
        .cluster
        .pd_client()
        .must_remove_peer(region.id, leader);
    ...
    suite.cluster.add_cloud_recv_filter(1, recv_filter);
    suite
        .cluster
        .pd_client()
    ...
    block_on(req_tx.send((req, WriteFlags::default()))).unwrap();
    sleep_ms(1000);
    suite.cluster.clear_recv_filters(1);
...
fn test_cdc_scan_ignore_gc_fence() {
    test_kv_format_impl!(test_cdc_scan_ignore_gc_fence_impl<ApiV2>);
}
...
fn test_cdc_scan_ignore_gc_fence_impl<F: KvFormat>() {
    ...
    let region_id = suite.cluster.get_region(b"xkey1").id;
    let (key, v1, v2) = (b"xkey1", b"value1", b"value2");
    ...
    suite.must_kv_prewrite(region_id, vec![mutation], key.to_vec(), start_ts1);

    let commit_ts1 = block_on(suite.cluster.pd_client().get_tso()).unwrap();
    suite.must_kv_commit(region_id, vec![key.to_vec()], start_ts1, commit_ts1);
    ...
    suite.must_kv_prewrite(region_id, vec![mutation], key.to_vec(), start_ts2);

    let commit_ts2 = block_on(suite.cluster.pd_client().get_tso()).unwrap();
    suite.must_kv_commit(region_id, vec![key.to_vec()], start_ts2, commit_ts2);
...
fn test_cdc_extract_rollback_if_gc_fence_set() {
    test_kv_format_impl!(test_cdc_extract_rollback_if_gc_fence_set_impl<ApiV2>);
}

fn test_cdc_extract_rollback_if_gc_fence_set_impl<F: KvFormat>() {
    let mut suite = TestSuite::new(1, F::TAG);

    let region_id = suite.cluster.get_region(b"xkey").id;
    let req = suite.new_changedata_request(region_id);
    let (mut req_tx, _, receive_event) = new_event_feed(suite.get_region_cdc_client(region_id));
    block_on(req_tx.send((req, WriteFlags::default()))).unwrap();
    ...
    suite.must_kv_commit(region_id, vec![key.to_vec()], start_ts3, commit_ts3);
...
// SoftState.
#[test]
#[ignore = "packet filtering wasn't supported yet (TODO!)"]
fn test_term_change() {
    let mut suite = CloudTestSuiteBuilder::new()
        .num_nodes(3)
        .after_cluster_bootstrapped(|c| c.cluster.wait_region_replicated(b"", 3))
        .build();
    ...
    let leader = suite.cluster.leader_of_region(region.id).unwrap();
    let leader_idx = peers.iter().position(|p| p.id == leader.id).unwrap();
    peers.swap(0, leader_idx);
    let transfee = peers[1].clone();

    suite.cluster.must_transfer_leader(region.id, transfee);
    // Simulate network partition.
    let recv_filter = Box::new(
        RegionPacketFilter::new(region.get_id(), leader.store_id).direction(Direction::Recv),
    );
    suite
        .cluster
        .add_cloud_recv_filter(leader.store_id, recv_filter);
    // Transfer leader to peer 3 and then change it back to peer 2.
    // Peer 1 would not get a new SoftState.
    suite
        .cluster
        .must_transfer_leader(region.id, peers[2].clone());
    suite
        .cluster
        .must_transfer_leader(region.id, peers[1].clone());
    suite.cluster.clear_recv_filters(leader.store_id);

    suite
        .cluster
        .pd_client()
        .must_remove_peer(region.id, peers[2].clone());
    let region = suite.cluster.get_region(&[]);
    let req = suite.new_changedata_request(region.id);
    let (mut req_tx, event_feed_wrap, receive_event) =
        new_event_feed(suite.get_region_cdc_client(region.id));
...
fn test_cdc_no_write_corresponding_to_lock() {
    test_kv_format_impl!(test_cdc_no_write_corresponding_to_lock_impl<ApiV2>);
}
...
fn test_cdc_no_write_corresponding_to_lock_impl<F: KvFormat>() {
    ...
    // Txn1 commit_ts = 15
    let mut m1 = Mutation::default();
    let k1 = b"xkey1".to_vec();
    m1.set_op(Op::Put);
    m1.key = k1.clone();
    m1.value = b"v1".to_vec();
    suite.must_kv_prewrite(region_id, vec![m1.clone()], k1.clone(), 10.into());
    suite.must_kv_commit(region_id, vec![k1.clone()], 10.into(), 15.into());

    // Txn2 start_ts = 15
    m1.value = b"v2".to_vec();
    suite.must_kv_prewrite(region_id, vec![m1.clone()], k1.clone(), 15.into());
    // unprotected rollback, no write is written
    suite.must_kv_rollback(region_id, vec![k1.clone()], 15.into());

    // Write a new txn
    m1.value = b"v3".to_vec();
    suite.must_kv_prewrite(region_id, vec![m1], k1.clone(), 20.into());
    suite.must_kv_commit(region_id, vec![k1], 20.into(), 25.into());
...
fn test_cdc_write_rollback_when_no_lock() {
    test_kv_format_impl!(test_cdc_write_rollback_when_no_lock_impl<ApiV2>);
}
...
fn test_cdc_write_rollback_when_no_lock_impl<F: KvFormat>() {
    ...
    suite.must_kv_rollback(region_id, vec![k1.clone()], 5.into());
...
fn test_resolved_ts_cluster_upgrading() {
    ...
    let mut suite = build_testsuite_from_cluster(cluster);
...
fn test_prewrite_without_value() {
    let mut suite = CloudTestSuiteBuilder::new()
        ...
        .after_cluster_bootstrapped(|c| c.cluster.get_pd_client().disable_default_operator())
        .build();
    let rid = suite.cluster.get_region(b"xkey").id;
    ...
    muts[0].key = b"xkeykey".to_vec();
    muts[0].value = large_value.clone();
    let w = try_kv_prewrite_pessimistic(&client, ctx.clone(), muts, b"xkeykey".to_vec(), 10);
    assert!(w.get_errors().is_empty(), "{:?}", w);
...
#[test]
#[ignore = "txn_source isn't supported by kvengine yet; maybe add them to `user_meta`"]
fn test_filter_loop() {
    test_kv_format_impl!(test_filter_loop_impl<ApiV2>);
}
...
fn test_filter_loop_impl<F: KvFormat>() {
    ...
    m1.key = k1.clone();
    m1.value = b"v1".to_vec();
    suite.must_kv_prewrite_with_source(region_id, vec![m1], k1.clone(), 10.into(), 1);
    let mut m2 = Mutation::default();
    let k2 = b"xk2".to_vec();
    m2.set_op(Op::Insert);
    m2.key = k2.clone();
    m2.value = b"v2".to_vec();
    suite.must_kv_prewrite_with_source(region_id, vec![m2], k2.clone(), 12.into(), 0);
    let mut events = receive_event(false).events.to_vec();
    match events.remove(0).event.unwrap() {
        Event_oneof_event::Entries(mut es) => {
    ...
    suite.must_kv_commit_with_source(region_id, vec![k1], 10.into(), 15.into(), 1);
    suite.must_kv_commit_with_source(region_id, vec![k2], 12.into(), 17.into(), 0);
    let mut events = receive_event(false).events.to_vec();
    match events.remove(0).event.unwrap() {
        Event_oneof_event::Entries(mut es) => {
            let events = es.take_entries().to_vec();
    ...
    suite.must_kv_prewrite_with_source(region_id, vec![m3], k3.clone(), 30.into(), 1);
    suite.must_kv_rollback(region_id, vec![k3], 30.into());
    let mut events = receive_event(false).events.to_vec();
    match events.remove(0).event.unwrap() {
        Event_oneof_event::Entries(mut es) => {
            let events = es.take_entries().to_vec();
    ...
    suite.must_kv_prewrite_with_source(region_id, vec![m4], k1.clone(), 40.into(), 1);
    suite.must_kv_commit_with_source(region_id, vec![k1], 40.into(), 42.into(), 1);
    let k2 = b"xk2".to_vec();
    let mut m5 = Mutation::default();
    m5.set_op(Op::Put);
    m5.key = k2.clone();
    m5.value = vec![b'4'; 5121];
    suite.must_kv_prewrite(region_id, vec![m5], k2.clone(), 44.into());
    suite.must_kv_commit(region_id, vec![k2.clone()], 44.into(), 46.into());
    let mut events = receive_event(false).events.to_vec();
    if events.len() == 1 {
        events.extend(receive_event(false).events.into_iter());
    }
...
#[test]
#[ignore = "This case was written for APIv1."]
fn test_flashback() {
    let mut suite = CloudTestSuiteBuilder::new()
        .num_nodes(1)
        .cfg_fun(|_id, cfg| cfg.resolved_ts.advance_ts_interval = ReadableDuration::millis(50))
        .build();

    let key = Key::from_raw(b"xkeya");
    ...
    for i in 0..2 {
        let (k, v) = (
            format!("xkeykey{}", i).as_bytes().to_vec(),
        ...
        // Commit
        let commit_ts = block_on(suite.cluster.pd_client().get_tso()).unwrap();
        suite.must_kv_commit(region_id, vec![k.clone()], start_ts1, commit_ts);
    }
    let (start_key, end_key) = (b"xkeykey0".to_vec(), b"xkeykey2".to_vec());
    // Prepare flashback.
                ...
                Event_oneof_event::Entries(entries) => {
                    ...
                    match e.get_type() {
                        EventLogType::Committed => {
                            ...
                            assert_eq!(e.get_key(), b"xkeykey1");
                            assert_eq!(event_counter, 1);
                        }
                        EventLogType::Commit => {
                            // Second entry should be a 2PC commit.
                            assert_eq!(e.get_key(), b"xkeykey0");
...
fn test_cdc_filter_key_range() {
    ...
    req_1_3.start_key = Key::from_raw(b"xkeykey1").into_encoded();
    req_1_3.end_key = Key::from_raw(b"xkeykey3").into_encoded();
    ...
    req_2_4.start_key = Key::from_raw(b"xkeykey2").into_encoded();
    req_2_4.end_key = Key::from_raw(b"xkeykey4").into_encoded();
    ...
    for case in &[
        ("xkeykey1", true, false, true /* commit */),
        ("xkeykey1", true, false, false /* rollback */),
        ("xkeykey2", true, true, true),
        ("xkeykey3", false, true, true),
        ("xkeykey4", false, false, true),
    ] {
        ...
        if case.3 {
            ...
            // Rollback
            suite.must_kv_rollback(region_id, vec![k.into_bytes()], start_ts);
            let mut events = receive_and_check_events(case.1, case.2);
            while let Some(event) = events.pop() {
                match event.event.unwrap() {
                    Event_oneof_event::Entries(entries) => {
...
fn test_cdc_partial_subscription() {
    let mut suite = CloudTestSuiteBuilder::new()
        .num_nodes(1)
        .cfg_fun(|_id, cfg| {
            configure_for_lease_read(cfg, Some(100), Some(10));
            configure_for_causal_ts(cfg, "42h", 1);
        })
        .after_cluster_bootstrapped(|cluster| {
            cluster.get_pd_client().disable_default_operator();
        })
        .build();
    let region = suite.cluster.get_region(b"xkey");
    let rid = region.id;

    let prewrite_tso = block_on(suite.cluster.pd_client().get_tso()).unwrap();
    let (k, v) = (b"xkey0".to_vec(), vec![b'x'; 16]);
    let mut mutation = Mutation::default();
    mutation.set_op(Op::Put);
    mutation.key = k.clone();
    mutation.value = v;
    ...
    req.set_start_key(Key::from_raw(b"y").into_encoded());
...
fn test_cdc_rollback_prewrites_with_txn_source() {
    let mut suite = CloudTestSuiteBuilder::new()
        ...
        .build();
    let region = suite.cluster.get_region(b"xkey0");
    let rid = region.id;
    let cf_tso = block_on(suite.cluster.pd_client().get_tso()).unwrap();
    ...
    req.set_start_key(Key::from_raw(b"xkeya").into_encoded());
    req.set_end_key(Key::from_raw(b"xkeyz").into_encoded());
    block_on(req_tx.send((req, WriteFlags::default()))).unwrap();
