...
use crate::{
    channel::{channel, Sink, CDC_CHANNEL_CAPACITY},
...
impl Service {
    ...
    fn handle_event_feed(
        ...
        let (event_sink, mut event_drain) =
            channel(conn_id, CDC_CHANNEL_CAPACITY, self.memory_quota.clone());
        ...
        info!("cdc connection created"; "downstream" => ctx.peer(), "conn_id" => ?conn_id);
...
impl ChangeData for Service {
    fn event_feed(
        ...
        self.handle_event_feed(ctx, stream, sink);
    }
...
mod tests {
    ...
    fn test_flow_control() {
        ...
        let (mut tx, mut rx) = client.event_feed_v2().unwrap();
