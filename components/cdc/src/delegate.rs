...
impl Downstream {
    ...
    pub fn sink_event(&self, mut event: Event, force: bool) -> Result<()> {
        ...
        if self.sink.is_none() {
            info!("cdc drop event, no sink"; "downstream_id" => ?self.id,
                "request_id" => ?self.req_id, "conn_id" => ?self.conn_id);
        ...
        match sink.unbounded_send(CdcEvent::Event(event), force) {
            ...
            Err(SendError::Disconnected) => {
                debug!("cdc send event failed, disconnected"; "downstream_id" => ?self.id,
                    "request_id" => ?self.req_id, "conn_id" => ?self.conn_id);
                Err(Error::Sink(SendError::Disconnected))
            }
            // TODO handle errors.
            Err(e @ SendError::Full) | Err(e @ SendError::Congested) => {
                info!("cdc send event failed, full"; "downstream_id" => ?self.id,
                    "request_id" => ?self.req_id, "conn_id" => ?self.conn_id);
                Err(Error::Sink(e))
            }
        }
...
impl Delegate {
    ...
    pub fn unsubscribe(&mut self, id: DownstreamId, err: Option<Error>) -> bool {
        ...
        if let Some(d) = self.remove_downstream(id) {
            if let Some(error_event) = error_event {
                info!("cdc unsubscribes downstream due to error";
                    "error" => ?error_event, "downstream" => ?d.peer,
                ...
                if let Err(err) = d.sink_error_event(region_id, error_event.clone()) {
                    warn!("cdc send unsubscribe failed";
                        "error" => ?err, "origin_error" => ?error_event,
        ...
        let send = move |downstream: &Downstream| {
            ...
            if let Err(err) = downstream.sink_error_event(region_id, error_event) {
                warn!("cdc send region error failed";
                    ...
                    "downstream_id" => ?downstream.id, "request_id" => ?downstream.req_id,
                    "region_id" => region_id, "conn_id" => ?downstream.conn_id);
            } else {
    ...
    fn sink_data(
        ...
        for mut req in requests {
            match req.get_cmd_type() {
                CmdType::Put => self.sink_txn_put(req.take_put(), &mut rows_builder)?,
        ...
        let txns = rows_builder.finish_build();
    ...
    fn sink_downstream_tidb(
        ...
        for d in &mut self.downstreams {
            if d.state.load().ready_for_change_events() {
                downstreams.push(d);
            }
        }
    ...
    fn stop_observing(&self) {
        info!("cdc stop observing"; "failed" => self.failed, "region_id" => self.region_id);
...
impl RowsBuilder {
    fn finish_build(self) -> Vec<RowInBuilding> {
...
mod tests {
    ...
    fn test_downstream_filter_entires() {
        ...
        let entries = rows_builder.finish_build();
        delegate
