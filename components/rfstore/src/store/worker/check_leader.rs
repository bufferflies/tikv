...
use raftstore::{coprocessor::CoprocessorHost, store::util::RegionReadProgressRegistry};
...
pub struct Runner {
    ...
    coprocessor: CoprocessorHost<kvengine::Engine>,
}
...
impl Runner {
    ...
    //     if key_range.get_start_key().is_empty() &&
    // key_range.get_end_key().is_empty() {         // Fast path to get the min
    // `safe_ts` of all regions in this store         self.region_read_progress.
    ...
    //         // per-transaction), also this branch won't entry because the request
    // key range         // is empty currently (in v5.1) keep this branch for
    // robustness and future use,         // so it is okay getting
    // `store_safe_ts` from `store_meta` (behide a mutex)         let meta =
    // self.store_meta.lock().unwrap();         meta.region_read_progress.
    // with(|registry| {             meta.region_ranges
    //             // get overlapped regions
    //             .range((Excluded(start_key), Unbounded))
    //             .take_while(|(_, id)| end_key >
    // enc_start_key(&meta.regions[*id]))             // get the min `safe_ts`
    //             .map(|(_, id)| {
...
mod tests {
    ...
    fn test_get_range_min_safe_ts() {
        fn add_region(meta: &Arc<Mutex<StoreMeta>>, id: u64, kr: KeyRange, safe_ts: u64) {
            ...
            meta.region_map
                .region_ranges
                .insert(enc_end_key(&region), id);
            meta.region_map.regions.insert(id, region);
