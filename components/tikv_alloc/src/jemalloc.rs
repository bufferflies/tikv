...
/// remove the postfix of threads generated by the thread pool.
/// For instance, YATP will append the id of the threads in a thread pool, which
/// will bring too many labels to the metric (and usually the memory usage
/// should be evenly distributed among these threads).
/// Fine-grained memory statistic is still available in the interface provided
/// for `tikv-ctl`.
fn trim_thread_suffix(s: &str) -> &str {
...
pub fn iterate_thread_allocation_stats(mut f: impl FnMut(&str, u64, u64)) {
    ...
    for (_, accessor) in thread_memory_map.iter() {
        let ent = collected
            .entry(trim_thread_suffix(&accessor.thread_name))
            .or_default();
...
mod tests {
    ...
    fn assert_delta(name: impl std::fmt::Display, delta: f64, a: u64, b: u64) {
        ...
        assert!(
            error < delta,
            "{}: the error is too huge: a={}, b={}, base={}, diff={}, error={}",
            name,
            a,
            b,
            base,
...
mod profiling {
    ...
    /// Set shared exclusive arena for the current anonymous thread to avoid
    /// contention.
    pub fn thread_allocate_shared_exclusive_arena() -> ProfResult<()> {
        if !ENABLE_THREAD_EXCLUSIVE_ARENA.load(Ordering::Relaxed) {
            return Ok(());
        }
    ...
    pub fn get_prof_sample() -> u64 {
        let rate: u64 = unsafe { tikv_jemalloc_ctl::raw::read(PROF_SAMPLE).unwrap() };
