...
use rfstore::{store::StoreRegionMeta, CdcHandle};
    ...
    S: StoreRegionMeta,
{
    ...
    fn handle_resolved_ts_advanced(
        ...
        let _now = tikv_util::time::Instant::now_coarse();
        for region_id in regions.iter() {
            if let Some(observe_region) = self.regions.get_mut(region_id) {
                if let ResolverStatus::Ready = observe_region.resolver_status {
                    let _ = observe_region.resolver.resolve(ts, ts_source.clone());
                }
            }
        }
    }
...
impl<T, S> Runnable for Endpoint<T, S>
where
    T: 'static + CdcHandle,
    S: StoreRegionMeta,
{
...
impl<T, S> RunnableWithTimer for Endpoint<T, S>
where
    T: 'static + CdcHandle,
    S: StoreRegionMeta,
{
