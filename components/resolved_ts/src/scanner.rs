...
use tikv::storage::mvcc::CloudReader;
use tikv_util::{
...
#[derive(Clone)]
pub struct ScannerPool<T> {
    workers: Arc<Runtime>,
    cdc_handle: T,
}

impl<T: 'static + CdcHandle> ScannerPool<T> {
    ...
    pub fn spawn_task(&self, mut task: ScanTask, concurrency_semaphore: Arc<Semaphore>) {
        ...
        let fut = async move {
            ...
            let apply_index = snap.get_applied_index();
            let mut reader = CloudReader::new(snap.snap, false);
            let mut done = false;
    ...
    async fn get_snapshot(task: &mut ScanTask, cdc_handle: T) -> Result<RegionSnapshot> {
