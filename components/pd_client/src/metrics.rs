...
lazy_static! {
    ...
    pub static ref STORE_SIZE_GAUGE_VEC: IntGaugeVec = register_int_gauge_vec!(
        ...
        "Size of storage.",
        &["type", "region_id", "keyspace_name", "storage_class"]
    )
    .unwrap();
    ...
    .unwrap();
    pub static ref STORE_ENGINE_MEM_SIZE_GAUGE_VEC: IntGaugeVec = register_int_gauge_vec!(
        "kv_engine_mem_size_bytes",
        "Sizes of memory used by engine",
        &["db", "type"]
    )
    .unwrap();
