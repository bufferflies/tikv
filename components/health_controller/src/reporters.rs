...
use crate::{
    slow_score::*,
    types::{InspectDuration, InspectFactor},
    HealthController, HealthControllerInner,
};

/// The parameters for building a [`RfStoreReporter`].
pub struct Config {
    /// The interval to tick the [`RfStoreReporter`].
    ///
    /// The `RfStoreReporter` doesn't tick by itself, the caller (the PD
    /// worker) is expected to tick it. But the interval is necessary in
    /// some internal calculations.
    pub inspect_raft_interval: Duration,
...
impl UnifiedSlowScore {
    fn new(cfg: &Config) -> Self {
        ...
        unified_slow_score.disk_factors.push(SlowScore::new(
            cfg.inspect_raft_interval, // timeout
    ...
    fn record_disk(
        ...
        // For disk disk_factors, we care about the process duration.
        let dur = duration.sum(false);
    ...
    #[inline]
    fn get(&self, factor: InspectFactor) -> &SlowScore {
        &self.disk_factors[factor as usize]
    }

    #[inline]
    fn get_mut(&mut self, factor: InspectFactor) -> &mut SlowScore {
        &mut self.disk_factors[factor as usize]
    }

    // Returns the maximum score of disk disk_factors.
    fn get_disk_score(&self) -> f64 {
        self.disk_factors
            .iter()
    ...
    fn get_network_score(&self) -> HashMap<u64, u64> {
    ...
    fn last_tick_finished(&self) -> bool {
        self.disk_factors.iter().all(SlowScore::last_tick_finished)
    }

    #[allow(dead_code)]
    fn record_timeout(&mut self, factor: InspectFactor) {
    ...
    fn tick(&mut self, factor: InspectFactor) -> SlowScoreTickResult {
        match factor {
            InspectFactor::Network => self.tick_network(),
            disk_factor => self.disk_factors[disk_factor as usize].tick(),
        }
    }
...
pub struct RfStoreReporter {
    health_controller_inner: Arc<HealthControllerInner>,
    slow_score: UnifiedSlowScore,
    is_healthy: bool,
}

impl RfStoreReporter {
    const MODULE_NAME: &'static str = "rfstore";

    pub fn new(health_controller: &HealthController, cfg: Config) -> Self {
        Self {
    ...
    pub fn record_disk_duration(
        ...
        if factor == InspectFactor::RaftDisk || factor == InspectFactor::KvDisk {
            ...
            self.health_controller_inner
                .update_rfstore_slow_score(self.slow_score.get_disk_score());
        }
    }
...
impl TestReporter {
    ...
    pub fn set_rfstore_slow_score(&self, slow_score: f64) {
        self.health_controller_inner
            .update_rfstore_slow_score(slow_score);
    }
}
...
mod tests {
    ...
    fn create_test_config() -> Config {
        Config {
            inspect_raft_interval: Duration::from_secs(1),
    ...
    #[test]
    fn test_unified_slow_score() {
        // Covers both disk factors and verifies independence.
        let cfg = create_test_config();

        // Case A: RaftDisk has timeouts, KvDisk does not.
        let mut unified = UnifiedSlowScore::new(&cfg);
        for i in 0..crate::slow_score::DISK_ROUND_TICKS {
            // Record for Raft: duration >= threshold -> timeout when not busy.
            let raft_id = unified.get(InspectFactor::RaftDisk).get_last_tick_id();
            let raft_dur = InspectDuration {
                wait_duration: None,
                process_duration: Some(Duration::from_secs(1)),
            };
            unified.record_disk(raft_id, InspectFactor::RaftDisk, &raft_dur, true);

            // Record for Kv: duration < threshold -> no timeout.
            let kv_id = unified.get(InspectFactor::KvDisk).get_last_tick_id();
            let kv_dur = InspectDuration {
                wait_duration: None,
                process_duration: Some(Duration::from_millis(100)),
            };
            unified.record_disk(kv_id, InspectFactor::KvDisk, &kv_dur, true);

            // Advance one tick for both factors.
            let raft_tick = unified.tick(InspectFactor::RaftDisk);
            let kv_tick = unified.tick(InspectFactor::KvDisk);

            if i + 1 == crate::slow_score::DISK_ROUND_TICKS {
                assert!(raft_tick.updated_score.is_some());
                assert!(kv_tick.updated_score.is_some());
            }
        }

        // After a full round, RaftDisk score should be > 1, KvDisk should stay at 1.
        let raft_score = unified.get(InspectFactor::RaftDisk).get();
        let kv_score = unified.get(InspectFactor::KvDisk).get();
        assert!(raft_score > 1.0);
        assert_eq!(kv_score, 1.0);
        assert_eq!(unified.get_disk_score(), raft_score);

        // Case B: KvDisk has timeouts, RaftDisk does not.
        let mut unified_b = UnifiedSlowScore::new(&cfg);
        for i in 0..crate::slow_score::DISK_ROUND_TICKS {
            // Record for Kv: duration >= threshold -> timeout when not busy.
            let kv_id = unified_b.get(InspectFactor::KvDisk).get_last_tick_id();
            let kv_dur = InspectDuration {
                wait_duration: None,
                process_duration: Some(Duration::from_secs(1)),
            };
            unified_b.record_disk(kv_id, InspectFactor::KvDisk, &kv_dur, true);

            // Record for Raft: duration < threshold -> no timeout.
            let raft_id = unified_b.get(InspectFactor::RaftDisk).get_last_tick_id();
            let raft_dur = InspectDuration {
                wait_duration: None,
                process_duration: Some(Duration::from_millis(100)),
            };
            unified_b.record_disk(raft_id, InspectFactor::RaftDisk, &raft_dur, true);

            // Advance one tick for both factors.
            let kv_tick = unified_b.tick(InspectFactor::KvDisk);
            let raft_tick = unified_b.tick(InspectFactor::RaftDisk);
        ...
        // After a full round, KvDisk score should be > 1, RaftDisk should stay at 1.
        let kv_score_b = unified_b.get(InspectFactor::KvDisk).get();
        let raft_score_b = unified_b.get(InspectFactor::RaftDisk).get();
        assert!(kv_score_b > 1.0);
        assert_eq!(raft_score_b, 1.0);
        assert_eq!(unified_b.get_disk_score(), kv_score_b);
    }
