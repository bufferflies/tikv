...
impl<C: Transport + Clone> Transport for SimulateTransport<C> {
...
impl<C: RaftStoreRouter> StoreRouter for SimulateTransport<C> {
    fn send(&self, msg: StoreMsg) {
        StoreRouter::send(&self.ch, msg)
    }
}

impl<C: RaftStoreRouter> ProposalRouter for SimulateTransport<C> {
    fn send(&self, cmd: RaftCommand) {
        ProposalRouter::send(&self.ch, cmd)
    }
}

impl<C: RaftStoreRouter> CasualRouter for SimulateTransport<C> {
    fn send(&self, region_id: u64, msg: CasualMessage) {
        CasualRouter::send(&self.ch, region_id, msg)
    }
}
...
impl<C: LocalReadRouter> LocalReadRouter for SimulateTransport<C> {
    fn read(&self, read_id: Option<ThreadReadId>, req: RaftCmdRequest, cb: Callback) -> Result<()> {
