...
ifeq ($(shell uname),Linux) # Macs don't have objcopy
	...
	objcopy --compress-debug-sections=zlib-gnu ${CARGO_TARGET_DIR}/release/cse-ctl
	objcopy --compress-debug-sections=zlib-gnu ${CARGO_TARGET_DIR}/release/tikv-worker
endif
...
x-build-dist: export X_PACKAGE=tikv-server tikv-ctl cse-ctl tikv-worker
x-build-dist:
...
x-build-dist-debug: export X_PACKAGE=tikv-server tikv-ctl cse-ctl tikv-worker
x-build-dist-debug:
	bash scripts/run-cargo.sh
